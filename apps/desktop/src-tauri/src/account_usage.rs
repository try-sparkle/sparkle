...
//!
//! Secrets stay in Rust. The per-account OAuth **access token** is read here — from the account's
//! own `<config_dir>/.credentials.json` if present, else from the macOS keychain — and is used ONLY
//! as the `Authorization: Bearer` on the one outbound call. It is NEVER logged, printed, persisted,
//! or handed to JS. Mirrors `auth.rs`'s keychain-read + authed-`ureq`-GET style.
//!
...
/// The precedence + fallback logic, with the two blocking IO operations injected. Order:
...
/// A creds file that is present but carries no usable token FALLS THROUGH to the keychain rather than
/// failing. `read_file`/`read_keychain` are the real implementations in production and fakes in tests.
fn read_access_token_with(
    ...
    if let Ok(contents) = read_file(&creds_path) {
        if let Some(token) = extract_access_token(&contents) {
            return Ok(token);
        }
    }
    ...
    for service in keychain_services(was_empty, &dir) {
        match read_keychain(&service, user) {
            Ok(password) => {
                if let Some(token) = extract_access_token(&password) {
                    return Ok(token);
                }
            }
    ...
    Err(last_err.unwrap_or_else(|| "no access token in stored credentials".to_string()))
}
