...
void sendNotification(NSString* title, NSString* subtitle, NSString* message, NSDictionary* options, const unsigned char* notificationId, BOOL shouldWait) {
    @autoreleasepool {
        ...
        if ([NSThread isMainThread]) {
            ...
            // it also unblocks on a real interaction without polling for it — `wait_for_click`
            // semantics are unchanged. The ONLY main-thread hop left is the one-shot
            // `resolveAutoDismiss` on genuine auto-dismiss, which does a zero-XPC run-loop drain
            // (to let a race-queued didActivate: win) and then reports the dismissal. `notificationId`
            // stays valid for the whole call: Rust keeps the id alive across this FFI boundary and
            // this branch runs synchronously inside sendNotification, so no NSData copy is needed.
            NSDate* pollStarted = [NSDate date];
            while (!rust_notification_is_done(notificationId)) {
                @autoreleasepool {
                    if (!rust_notification_is_delivered(notificationId)) {
                        // Delivery not yet confirmed — the async delivery window would otherwise look
                        // like an immediate dismiss. Give up only after the delivery timeout.
                        if (-[pollStarted timeIntervalSinceNow] > kDeliveryTimeoutSecs) {
            ...
            // A click/dismiss delegate callback lands on main and sets `done`; make sure we don't
