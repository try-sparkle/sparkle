  ...
  if (variant === "mini") {
    return (
      ...
      >
        ...
        {/* NO PREVIEW SEGMENT, and this is a decision rather than an omission. The founder,
            2026-08-19: a preview does not belong as a third peer column beside Build and Plan; it
            belongs as a CARD IN THE CONCIERGE CHAT (`Concierge/PreviewCards.tsx`), where it is a
            snapshot you click to open the real localhost url.

            So the strip is back to two, and there is nothing to gate: `previewEnabled` and
            `onPickPreview` are gone with it, along with the preview work mode and the pane the
            segment revealed. Re-adding a segment here would re-create the peer column he rejected —
            if a preview needs a bigger surface, that is a bigger CARD, not a third mode. */}
      </span>
    );
  }
  // THE CHEVRON VARIANT never carried a Preview button, so nothing here changed when the mini
  // strip lost its own — the tessellation (`chevronClip`'s notch/point pairing) is written for a
  // two-button strip and now both variants agree on two.
  return (
    <div
      style={{
        display: "flex",
