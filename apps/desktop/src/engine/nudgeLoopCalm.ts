 ...
 *
 * ⚠️ THE ARGUMENT ABOVE LEANS ON THE LADDER BEHAVING, which this module cannot check from here: the
 * escalation level lives in Rust and is not published to the frontend today. Plumbing it through and
 * gating on it directly is the durable fix, and it travels with the still-open question of what the
 * nudger should DO once it notices its own pings are not landing. Until then this set stays as
 * narrow as it can be while still covering the case that was actually measured.
 */
...
export function withNudgeLoopCalm<T extends { id: string }>(
  ...
  for (const a of agents) {
    ...
    if (!isNudgeLoopCalmed(p, thrashOf(a.id))) continue;
