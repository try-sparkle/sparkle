...
// "first" now depends on the worktree reading too. `holdsWorkOf` used to be optional, so a caller
// that dropped it was not a type error and, before this file, not a test failure either.
//
// ⚠️ THIS FILE IS NOT COVERAGE FOR ANY CALL SITE, and reading it as such is the mistake bead
// `sparkle-l5fi7` was filed about. It drives the PURE FUNCTION with the accessor passed by hand,
// which proves the function READS it and says nothing about whether AgentSidebar HANDS IT OVER —
// deleting `headHoldsWorkFor` from `firstRenderedRowId` left this whole file green. The call sites
// are covered by AgentSidebar.ladderAccessorWiring.test.tsx (the column and selection) and
// conciergeTools/sidebarView.test.ts (the concierge), each mutation-checked by deleting the
// argument. The parameters are also REQUIRED now, so the omission is an arity error too.
...
describe("firstLadderRowId — the `local_none` rung moves what 'first' means", () => {
  it("picks the CLEAN row, because `local_none` sorts above `local_uncommitted`", () => {
    ...
    expect(
      firstLadderRowId(AGENTS, "build", stageOf, statusOf, allBandsVisible(), holdsWorkOf, undefined),
    ).toBe("clean");
  });

  it("with an `undefined` accessor it returns the ARRAY-order row — the drift this guards", () => {
    ...
    expect(firstLadderRowId(AGENTS, "build", stageOf, statusOf, allBandsVisible(), undefined, undefined)).toBe("dirty");
  });
  ...
  it("an UNREAD row does not jump the queue", () => {
    ...
    expect(
      firstLadderRowId(AGENTS, "build", stageOf, statusOf, allBandsVisible(), () => undefined, undefined),
    ).toBe("dirty");
  });
});
...
// than `holdsWorkOf` does. `crossRepoOf` was an OPTIONAL trailing parameter, so deleting the argument
// at a call site was neither a type error nor, without this block, a test failure: the feature would
// silently revert to exactly the state roborev 67500 found, where selection pointed at a row the
// column was not rendering. Same paired shape as the tests above, and — see the header — the same
// limit: this is a statement about the FUNCTION, not about any caller of it.
describe("firstLadderRowId — the `tracked_elsewhere` rung sorts above everything", () => {
  ...
  it("with an `undefined` accessor it returns the ARRAY-order row — the drift this guards", () => {
    ...
    expect(
      firstLadderRowId(ROWS, "build", stageOf, statusOf, allBandsVisible(), () => false, undefined),
    ).toBe("local");
  });
