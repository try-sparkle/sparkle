...
describe("aiProviderStore", () => {
  ...
  it("records an outage with its reason and timestamp", () => {
    useAiProviderStore.getState().noteOutage("provider_unfunded", 1234);
    expect(useAiProviderStore.getState().outage).toEqual({ reason: "provider_unfunded", at: 1234 });
  });
  ...
  it("expires a stale observation, so a forgotten clear cannot strand a false banner", () => {
    ...
    const outage = { reason: "provider_unfunded" as const, at: 1_000 };
  ...
  it("re-stamps a still-live outage so it cannot age out mid-outage", () => {
    ...
    useAiProviderStore.getState().noteOutage("provider_unfunded", 0);
    useAiProviderStore.getState().noteOutage("provider_unfunded", OUTAGE_MAX_AGE_MS / 2);
  ...
  it("keeps the same object identity while the reason is unchanged", () => {
    ...
    useAiProviderStore.getState().noteOutage("provider_unfunded", 1);
    const first = useAiProviderStore.getState().outage;
    // Within the refresh window, so identity is preserved (no subscriber churn).
    useAiProviderStore.getState().noteOutage("provider_unfunded", 999);
    expect(useAiProviderStore.getState().outage).toBe(first);
  });

  it("replaces the record when the reason actually changes", () => {
    useAiProviderStore.getState().noteOutage("provider_unfunded", 1);
    useAiProviderStore.getState().noteOutage("provider_key_rejected", 2);
    expect(useAiProviderStore.getState().outage).toEqual({ reason: "provider_key_rejected", at: 2 });
  });
