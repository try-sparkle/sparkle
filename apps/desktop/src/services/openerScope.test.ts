...
// added `${WEB_BASE_URL}/foo` opener URL is automatically checked against the scope — otherwise a
...
function openedPaths(): string[] {
  ...
  // Match any `${WEB_BASE_URL}/some/path` template literal and capture the path. The opened URLs
  // are named constants (SIGN_IN_URL/PAYWALL_URL) handed to openUrl(), so we key off the base
  // interpolation rather than the openUrl() call site.
  const paths = [...src.matchAll(/`\$\{WEB_BASE_URL\}(\/[^`]*)`/g)]
    .map((m) => m[1])
