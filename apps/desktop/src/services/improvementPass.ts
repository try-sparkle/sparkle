...
export function hourlyMissionPrompt(
  ...
  return [
    ...
    "`scripts/retro-inbox-triage.sh`, NOT a raw `bd list`: the inbox is past 400 open items, so a",
    "raw list is unreadable in one pass and you would re-read the same first screen every hour.",
    "Triage ranks it worst-first and, critically, marks the beads whose fix has already MERGED or",
    "already LANDED on main — picking one of those means re-investigating finished work. It is a dry",
    "run that writes nothing, so it is always safe to run. Then `bd show` only the item you pick.",
    "For each item, file a new bead or enrich an",
