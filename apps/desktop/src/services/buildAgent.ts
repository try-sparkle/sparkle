...
export function retroEmissionProtocol(): string {
  return [
    ...
    "- `unfiled:parked-*` is DEFERRED, NOT declined. The point was written to the drop log and the",
    "  next retro run re-files it automatically — do NOT run it again by hand, because the dedupe",
    "  path reads a second run as a genuine recurrence and escalates the bead's priority for one",
    "  sighting. `unfiled:no-bd` / `unfiled:lost` (each with a `retro LOST` line on stderr) are the",
    "  tokens that really did lose it, and only those are worth re-running.",
