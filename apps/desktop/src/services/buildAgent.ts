...
export function orchestrationPersona(opts: {
  ...
  return [
    ...
    "  a spawn that goes over stalls ~10 minutes and then fails. Assume less is available than the",
    "  cap, so you meet that stall rarely; you cannot avoid it by calculation.",
    ...
    "    • An ERROR or timeout → the machine was FULL and THAT UNIT WAS NOT STARTED. Do not retry it",
    "      and do not spawn anything else until a `spin_down_worker` of yours has freed a slot. Then",
    "      re-spawn exactly that unit — it is still unclaimed work.",
    ...
    "- Batch workflow: (1) spawn your batch one worker at a time, halting on the first ERROR per THE",
    "  RULE — the cap is shared, so do not assume all of it is yours; (2) `wait_for_workers(...)` on",
    "  the workers that DID start, (3) merge + `spin_down_worker` each to free its slot, (4) spawn the",
    "  next batch, starting with any unit whose spawn errored.",
