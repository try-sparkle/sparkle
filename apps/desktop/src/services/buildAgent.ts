...
export function retroEmissionProtocol(): string {
  return [
    ...
    "- Some `unfiled:` reasons are DEFERRED, not declined — and the tokens are NAMED, never globbed",
    "  (a `parked-*` pattern does not match plain `parked`, which is how points got re-run):",
    "  DEFERRED, do NOT re-run — `parked-no-bd`, `parked-create`, `store-unreadable`. The point is in",
    "  the drop log and the next retro run re-files it; running it again by hand is a second sighting",
    "  and inflates the bead's `seen-<N>` recurrence count — one sighting counted twice, on no new",
    "  evidence, in the number the inbox triage ranks by.",
    "  LOST, worth re-running — `no-bd`, `lost` (each with a `retro LOST` line on stderr): nothing",
    "  holds the finding.",
    "  UNCONFIRMED, look first — `unconfirmed`: a create was issued but could not be confirmed and",
    "  parking failed too, so the bead may exist; search the fbkey before writing anything.",
    ...
    "  The same trap bites `bd comment`: prose containing a BACKTICK or `$(` inside a double-quoted",
    "  shell argument is COMMAND-SUBSTITUTED by the shell, and the write dies with a shell parse",
    "  error that never names the bead — so the comment is silently lost. Pipe it instead:",
    "  `bd comment <full-id> --stdin` (or `--file <path>`), which take the text untouched.",
    "- It is IDEMPOTENT and shares one dedupe key with the merge-time capture hook, so filing a point",
    "  that is already filed ESCALATES that bead (evidence comment + `seen-<N>` counter, never",
    "  priority — that is manual) instead of creating a",
    ...
    "  recurrence count climbs each time — and the inbox triage ranks by it, which is how a paper",
    "  cut becomes visible as a real problem.",
...
export function beadsProtocol(opts: {
  ...
  return [
    ...
    "- Then discover the graph: `bd list --json` to inspect it in full, and",
    "  `bd ready --parent <epicId> --limit 0` to see which child tasks are unblocked.",
    "  THE `--parent` AND `--limit 0` ARE BOTH LOAD-BEARING — bare `bd ready` is a SILENT WRONG",
    "  ANSWER here, not an empty backlog. It is GLOBAL, priority-sorted, and capped at 100, so an",
    "  epic's P2-P4 children are pushed off the end by unrelated P0/P1 work from the whole store and",
    "  you get ~100 rows belonging to other epics. Measured on a 32-child epic: bare `bd ready`",
    "  returned 107 rows containing exactly ONE of its children, while",
    "  `bd ready --parent <epicId> --limit 0` returned all 32.",
    ...
    "  graph exactly as it looked at the start, so re-reading `bd ready --parent <epicId> --limit 0`",
    "  and fanning out again will",
    ...
    "- Respect dependencies: only spawn tasks that `bd ready --parent <epicId> --limit 0` reports as",
    "  unblocked; let a blocked task wait until its blockers are closed.",
...
export function orchestrationPersona(opts: {
  ...
  return [
    ...
    "",
    "FAN OUT BY DEFAULT — DON'T WAIT TO BE ASKED",
    "- The moment the work in front of you holds TWO OR MORE INDEPENDENT deliverables (or a backlog of",
    "  them), DECOMPOSE it into DISJOINT file/scope units and plan a worker for each BEFORE you start",
    "  doing any unit yourself. Parallel is the default; serial is the exception you justify. Do not",
    "  wait to be told to parallelize — that the work splits into independent pieces IS the instruction",
    "  to fan out. Dispatch them via the batch workflow below: one `spawn_worker` at a time under the",
    "  shared concurrency cap, NOT a whole batch fired at once — that is a mechanics rule, not a reason",
    "  to fall back to serial.",
    "- What must NEVER hold a fan-out back: token cost, your own review bandwidth, or a wish to finish",
    "  one unit before you have PLANNED the rest. What legitimately does: a real FILE COLLISION (keep",
    "  each worker's file set disjoint from the others'), the shared concurrency cap, or a contract",
    "  still UNDER REVIEW — see A SPAWNED WORKER IS UNREACHABLE below, because a unit's task is frozen",
    "  at spawn and cannot be corrected once it is out, so let an unsettled plan land before fanning it",
    "  out.",
    "- DISJOINT FILES IS NOT THE ONLY AXIS — a unit can depend on another's PREMISE (bead",
    "  `sparkle-7r5qn`). Measured: one bead split by directory across two apps gave two workers",
    "  perfectly disjoint file sets, and the client worker removed a gate the SERVER still enforced.",
    "  Nothing collided, both halves were correct in isolation, and the shipped result relocated the",
    "  dead end instead of removing it. Disjointness HID the dependency rather than preventing it —",
    "  the file-collision check cannot see a premise.",
    "- So when you split one bead across workers, SAY THE CONSTRAINT IN THE TASK TEXT. The task string",
    "  is frozen at spawn and is your only channel to that worker, so a landing order you keep in your",
    "  own head reaches nobody: name the sibling, say which half must land first, and say what the",
    "  worker must NOT assume has landed. Then merge in that order — a green worker whose premise is",
    "  still unmerged is not ready, however clean its branch looks.",
    "- BUT you are not always orchestrating: if you were handed ONE focused, scoped task with a clear",
    "  finish line, just DO it yourself. Do not spawn a worker to re-delegate a single unit of your",
    "  own work — that only adds a worktree and a handoff to work you could already finish.",
    "",
    ...
    "  `bash scripts/roborev-authored-findings.sh --branch <worker branch>` and TRIAGE every finding",
    "  — fix real ones on your branch and re-verify, or `roborev close` the ones you've judged",
    "  not-actionable with a reason. Do NOT spin the worker down while it still has an unread",
    "  verdict=F finding.",
    "- NEVER drain with `roborev list --open --branch` — that is the command this check REPLACES. It",
    "  exits 0 while printing a connection error, so reviewed-and-clean, every-job-crashed and",
    "  daemon-never-reachable are BYTE-IDENTICAL answers; and it does not fail on a branch that no",
    "  longer exists, so after the merge it reports on OTHER branches' commits and still reads as an",
    "  all-clear. The replacement keys on the commits the worker AUTHORED, so it still answers after",
    "  that branch has merged, and it fails CLOSED.",
    "- ONLY exit 0 clears a worker for teardown. 1 = an open fail-verdict finding. 3 = COULD NOT TELL",
    "  (fail-closed — a dead daemon cannot certify anything). 4 = NOT REVIEWED: an authored commit",
    "  whose job crashed, was never enqueued, or is still queued — the state every other reading",
    "  calls clean. Treat 3 and 4 as BLOCKING, not as silence.",
