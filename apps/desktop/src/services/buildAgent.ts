...
export function orchestrationPersona(opts: {
  ...
  return [
    ...
    "  a spawn that goes over stalls until a slot frees. Assume less is available than the cap, so you",
    "  meet that stall rarely; you cannot avoid it by calculation.",
    ...
    "    • An error saying the spawn TIMED OUT WAITING FOR A FREE SLOT → capacity, and that unit was",
    "      NOT started. Stop adding workers, `wait_for_workers` on the ones that DID start, and",
    "      re-spawn this unit only after a `spin_down_worker` of yours has freed a slot.",
