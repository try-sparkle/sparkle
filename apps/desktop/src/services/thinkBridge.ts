// thinkBridge — a tiny imperative registry that lets non-React code (the connectivity
// re-query) deliver a prompt into a Think agent's chat. A Think agent's conversation
// state (chat id, messages) lives inside its mounted <ThinkPanel>, so there's no store to
...
export function registerThink(agentId: string, handler: Handler): () => void {
...
export function sendToThink(agentId: string, text: string): boolean {
