...
import { registerThink, sendToThink } from "./thinkBridge";

// The bridge lets the connectivity re-query reach a Think agent whose chat state
...
describe("thinkBridge", () => {
  ...
  it("delivers text to a registered handler and reports success", () => {
    const fn = vi.fn();
    track(registerThink("a1", fn));
    const ok = sendToThink("a1", "hi");
    expect(ok).toBe(true);
    expect(fn).toHaveBeenCalledWith("hi");
  });

  it("reports failure when no handler is registered for the agent", () => {
    expect(sendToThink("missing", "hi")).toBe(false);
  });

  it("stops delivering after the handler unregisters", () => {
    const fn = vi.fn();
    const off = track(registerThink("a2", fn));
    off();
    expect(sendToThink("a2", "hi")).toBe(false);
  ...
  it("unregister only removes its own handler, not a newer one for the same id", () => {
    const first = vi.fn();
    const second = vi.fn();
    const offFirst = track(registerThink("a3", first));
    track(registerThink("a3", second)); // remount replaced the handler
    offFirst(); // stale cleanup from the old mount must not drop the new handler
    expect(sendToThink("a3", "yo")).toBe(true);
